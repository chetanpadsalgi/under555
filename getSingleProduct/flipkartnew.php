...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) 
	...
	if(strpos($data['url'] , "flipkart")){
		...
		/********************/
		if($data["price"] == "on"){
			$out["price"] = get_price_mrp($output);
		}
		
		if($data["price_special"] == "on"){
			$out["price_special"] = get_price($output);
		}
...
function get_price_mrp($html){
		$res =  explode('MRP: <span class="price">Rs.' , $html);
       if(count($res) > 1){
       		$res = explode('</' , $res[1]);
       		if(count($res) > 1){
