...
class ControllerModuleBossRevolutionSlider extends Controller {
	...
	$data_template = $this->cache->get('boss_revolutionslider.' . (int)$this->config->get('config_language_id'));
		...
		if(!$data_template){
			...
			$this->render();
			//$this->cache->set('boss_revolutionslider.' . (int)$this->config->get('config_language_id'),$this->render());
